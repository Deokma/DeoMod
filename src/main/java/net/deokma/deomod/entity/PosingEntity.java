...
import net.deokma.deomod.error.LoadSkinException;
import net.deokma.deomod.util.SkinUtil;
...
import static net.deokma.deomod.mixin.accessor.PlayerEntityAccessor.getMAIN_ARM;
import static net.deokma.deomod.mixin.accessor.PlayerEntityAccessor.getPLAYER_MODEL_PARTS;
...
public abstract class PosingEntity extends ServerPlayerEntity {
    ...
    public PosingEntity(ServerPlayerEntity player, GameProfile gameProfile) {
        ...
        if(FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
            DeoMod.LOGGER.info("FabSit posing client side - attempting to fetch skin manually");
    ...
    private void fetchSkinAndUpdate() {
        ...
        try {
            ...
            DeoMod.LOGGER.error("Could not load skin for NPC for " + this.player.getName().getString() + ", got " + e);
        }
        ...
        DeoMod.LOGGER.info("Updated skin for " + this.player.getName().getString());
    }
